...
Game::Game(int height,int width){
	...
	for(int i=0;i<n0;i++){
		Enemy0 e = Enemy0(win,(1 + rand()%(height-2)),(1 + rand()%(width-2)),'0',9,1); //create one enemy
		enemies0 = Game::head_insert_enemy0(enemies0,e,i); //add the enemy into the list
	}

	n1 = rand()%k; //number of enemies of type 1
	for(int i=0;i<n1;i++){
		Enemy1 e = Enemy1(win,(1 + rand()%(height-2)),(1 + rand()%(width-2)),'1',9,2); //create one enemy
		enemies1 = Game::head_insert_enemy1(enemies1,e,i); //add the enemy into the list
	}

	n2 = rand()%k; //number of enemies of type 2
	for(int i=0;i<n2;i++){
		Enemy2 e = Enemy2(win,(1 + rand()%(height-2)),(1 + rand()%(width-2)),'2',15,3); //create one enemy
